...
class Markdown(commands.Cog):
	...
	@todo.command(name='ajouter', aliases=['add'], help='Ajouter un élément à la todo_list')
	async def add(self, ctx: commands.Context, *, txt: str):
		"""Ajouter un élément à la todo_list."""
		with open(path_todo_list, 'a') as todo_list:
			list_multi_index = MultiIndex.extract(path_file=path_todo_list, regex = r'{[\d\.]*}')
	...
	@todo.command(name='retirer', aliases=['remove', 'supprimer'], help='Supprime une ligne de la todo-list')
			...
			for line in lines:
				...
				if last_child is not None:
					...
					if f'{{{father_multi_index}}}' in line:
						txt_formatted = f'{{{father_multi_index.make_child(i=1)}}} ' + txt + '\n'
						todo_list.write(fmdcheck(txt_formatted, level=lvl))

	@todo.command(name='valider', aliases=['fini', 'check'], help='Coche une checkbox dans la todo-add')
