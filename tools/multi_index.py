...
class MultiIndex:
	...
	@classmethod
	def extract(cls, path_file: str, regex: str = r'{[\d\.]*}', max_per_line: int = 1) -> list:
		...
		with open(path_file, 'r') as file:
			...
			for ligne in file:
				...
				if len(list_founded) > 0:
					if len(list_founded) <= max_per_line: # j'en est pas trouver beaucoup
						list_multi_index += [cls(m_i) for m_i in list_founded]
					else: # j'en est trouver trop
						list_multi_index += [cls(m_i) for m_i in list_founded[:max_per_line]]
	...
	def __init__(self, txt):
		...
		if isinstance(txt, str):
			if (txt[0],txt[-1]) == ('{','}'): # car de la forme {1.2.3}
				...
				txt = txt[:-1]
			self.levels = [int(i) for i in txt.split('.')] # list -> [1, 3, 5] pour 1.3.5
	...
	def children(self, list_of_multi_index: list) -> list:
		...
		for node in list_of_multi_index:
			match = True
			if node.level == (father_level + 1): # Bon nombre de niveau pour être un enfant (bonne génération)
				for lvl in range(father_level):
					if node[lvl] != self[lvl]: # Chaque niveau doivent se correspondre (même lignée)
						match = False
						break
				if match:
