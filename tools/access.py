	...
	def admin(self, func):
		...
		async def decorated(obj, ctx: commands.Context, *args, **kwargs):
			if ctx.author.id in masters_id:
				...
				await ctx.send('> '+self.rejection+' Il s\'agit d\'une commande administrateur.')
	...
	def has_min_role(self, roles_authorized, nb_min: int = 1):
		...
		def deco(func):
			...
			async def decorated(obj, ctx: commands.Context, *args, **kwargs):
				...
				if len(self.intersection([r.name for r in ctx.author.roles], roles_authorized)) >= nb_min:
					return await func(obj, ctx, *args, **kwargs)
				else:
					rejection = '```md\n' + self.rejection + \
